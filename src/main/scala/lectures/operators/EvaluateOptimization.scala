...
object EvaluateOptimization extends App with Data {
  ...
  val partialComp = CurriedComputation.curriedComputation(filterData) _
  // ВЫПОЛНИТЬ В ЦИКЛЕ ОТ 1 ДО 100 CurriedComputation.partiallyAppliedCurriedFunction
  for (_ <- 1 to 100) {
    partialComp(dataArray)
  }
  ...
  val partialFunc = FunctionalComputation.functionalComputation(filterData)
  // ВЫПОЛНИТЬ В ЦИКЛЕ ОТ 1 ДО 100 FunctionalComputation.filterApplied
  for (_ <- 1 to 100) {
    partialFunc(dataArray)
  }
